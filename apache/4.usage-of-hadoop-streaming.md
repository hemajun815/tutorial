...
4. 执行命令：
	```console
	hadoop jar $HADOOP_PREFIX/share/hadoop/tools/lib/hadoop-streaming-2.8.1.jar -input /wordcount/words -output /wordcount/output -files mapper.py reducer.py -mapper "python mapper.py" -reducer "python reducer.py" -D mapreduce.reduce.tasks=1 -D mapreduce.job.name="hs-test"
	```
